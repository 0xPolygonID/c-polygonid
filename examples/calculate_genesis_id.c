...
main() {
    int ret_val = 0;
    char *input = read_file("calculate_genesis_id_in.json");
    if (!input) {
        ret_val = 1;
        goto cleanup;
    }
    char *want_output = read_file("calculate_genesis_id_out.json");
    if (!want_output) {
        ret_val = 1;
        goto cleanup;
    }
    ...
    bool ok = PLGNCalculateGenesisID(&resp, input, &status);
    if (!ok) {
