...
func VerifyAuthResponse(ctx context.Context, cfg EnvConfig, in []byte) (protocol.AuthorizationResponseMessage, error) {
	...
	err = json.Unmarshal(p.AuthResponse, &authResp)
	if err != nil {
		return protocol.AuthorizationResponseMessage{},
...
// FullVerify verifies an authorization response using the provided environment configuration and token
func FullVerify(ctx context.Context, cfg EnvConfig, in []byte) (protocol.AuthorizationResponseMessage, error) {
	var p proofVerification
	err := json.Unmarshal(in, &p)
	if err != nil {
		return protocol.AuthorizationResponseMessage{},
